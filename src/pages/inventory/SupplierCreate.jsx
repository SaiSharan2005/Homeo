...
import { createSupplier } from '../../services/inventory/supplier';
...
const SupplierCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error('Failed to create supplier: ' + (err.message || 'Unknown error'));
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => navigate('/inventory/suppliers')} className="px-4 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Back</button>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-lg shadow p-6">
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={()=>navigate('/inventory/suppliers')} className="px-4 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Cancel</button>
          <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Create Supplier</button>
        </div>
      </form>
    </div>
  );
