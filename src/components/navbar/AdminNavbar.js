  ...
  return (
    <nav className="bg-teal-700 p-4 flex justify-between items-center px-7 md:px-10 lg:px-20 shadow-lg">
      ...
      <div className="flex items-center space-x-6">
        ...
        <button className="text-white" onClick={() => navigate('/inventory-items')}>
          Inventory
        </button>
        ...
        {isOpen && (
          <div ref={mobileMenuRef} className="md:hidden absolute w-48 top-16 right-4 rounded-md bg-white shadow-lg p-4 flex flex-col items-start space-y-4 z-50 border border-gray-200">
            ...
            {/* Mobile Inventory Menu Item */}
            <button className="text-gray-800" onClick={() => navigate('/inventory-items')}>
              Inventory
            </button>
            <button className="text-gray-800" onClick={() => { localStorage.clear(); navigate("/") }}>
              Log Out
            </button>
          </div>
        )}
